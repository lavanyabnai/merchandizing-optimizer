...
const patcheventsSchema = z.object({
...
const app = new Hono()
  .get('/', zValidator('query', z.object({})), async (c) => {
    ...
    const data = await db
      .select({
        ...
        occurrenceType: events.occurrenceType,
        occurrenceTime: events.occurrenceTime,
        triggerEventName: events.triggerEventName,
  ...
  .post(
    '/',
    zValidator('json', insertEventsSchema),
    async (c) => {
      try {
        ...
        let data;
        try {
          [data] = await db.insert(events).values(values).returning();
        } catch (dbError: any) {
  ...
  .post(
    '/bulk-create',
    zValidator('json', z.array(insertEventsSchema.omit({ id: true }))),
    async (c) => {
      const values = c.req.valid('json');
  ...
  .post(
    ...
    async (c) => {
      ...
      try {
        const data = await db
          .delete(events)
          .where(inArray(events.id, ids)) // Use ids directly
  ...
  .patch(
    ...
    zValidator('json', patcheventsSchema),
    async (c) => {
      try {
        const { id } = c.req.valid('param');
        const values = c.req.valid('json');
