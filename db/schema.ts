...
// // AI Analytics Results Cache
// export const aiAnalyticsCache = pgTable('ai_analytics_cache', {
//   id: serial('id').primaryKey(),
//   userId: varchar('user_id', { length: 255 }).notNull(),
//   queryHash: varchar('query_hash', { length: 64 }).notNull(), // SHA-256 hash of the query
...
//   createdAt: timestamp('created_at').defaultNow()
// }, (table) => ({
//   userQueryIdx: index('idx_ai_cache_user_query').on(table.userId, table.queryHash),
//   expiresIdx: index('idx_ai_cache_expires').on(table.expiresAt),
//   analysisTypeIdx: index('idx_ai_cache_analysis_type').on(table.analysisType)
// }));

// export const insertAIAnalyticsCacheSchema = createInsertSchema(aiAnalyticsCache);
...
//   insightType: varchar('insight_type', { length: 50 }).notNull(), // 'trend', 'anomaly', 'recommendation', 'alert'
//   title: varchar('title', { length: 255 }).notNull(),
...
//   conversationIdx: index('idx_ai_insights_conversation').on(table.conversationId),
//   typeIdx: index('idx_ai_insights_type').on(table.insightType),
//   statusIdx: index('idx_ai_insights_status').on(table.status),
//   createdAtIdx: index('idx_ai_insights_created_at').on(table.createdAt)
// }));

// export const insertAIInsightSchema = createInsertSchema(aiInsights);

// // AI Query Templates
// export const aiQueryTemplates = pgTable('ai_query_templates', {
//   id: serial('id').primaryKey(),
//   name: varchar('name', { length: 255 }).notNull(),
//   category: varchar('category', { length: 50 }).notNull(), // 'demand', 'supply', 'customer', 'network'
//   description: text('description'),
//   template: text('template').notNull(),
//   parameters: jsonb('parameters'), // Dynamic parameters for the template
...
//   updatedAt: timestamp('updated_at').defaultNow()
// }, (table) => ({
//   categoryIdx: index('idx_ai_templates_category').on(table.category),
//   publicIdx: index('idx_ai_templates_public').on(table.isPublic),
//   usageIdx: index('idx_ai_templates_usage').on(table.usageCount)
// }));

// export const insertAIQueryTemplateSchema = createInsertSchema(aiQueryTemplates);

// // AI User Preferences
// export const aiUserPreferences = pgTable('ai_user_preferences', {
//   userId: varchar('user_id', { length: 255 }).primaryKey(),
//   defaultAnalysisType: varchar('default_analysis_type', { length: 50 }),
//   preferredVisualizations: jsonb('preferred_visualizations').default('[]'),
//   autoSuggestions: boolean('auto_suggestions').default(true),
//   saveHistory: boolean('save_history').default(true),
//   exportFormat: varchar('export_format', { length: 20 }).default('json'),
...
//   feedbackType: varchar('feedback_type', { length: 20 }).notNull(), // 'helpful', 'not_helpful', 'incorrect'
//   rating: integer('rating'), // 1-5 scale
//   comment: text('comment'),
//   metadata: jsonb('metadata'),
//   createdAt: timestamp('created_at').defaultNow()
// }, (table) => ({
//   userIdIdx: index('idx_ai_feedback_user_id').on(table.userId),
//   conversationIdx: index('idx_ai_feedback_conversation').on(table.conversationId),
//   typeIdx: index('idx_ai_feedback_type').on(table.feedbackType)
// }));

// export const insertAIFeedbackSchema = createInsertSchema(aiFeedback);

// // AI Saved Reports
// export const aiSavedReports = pgTable('ai_saved_reports', {
//   id: serial('id').primaryKey(),
//   userId: varchar('user_id', { length: 255 }).notNull(),
...
//   content: jsonb('content').notNull(), // Full report content including visualizations
...
//   typeIdx: index('idx_ai_reports_type').on(table.reportType),
//   publicIdx: index('idx_ai_reports_public').on(table.isPublic),
//   createdAtIdx: index('idx_ai_reports_created_at').on(table.createdAt)
// }));

// export const insertAISavedReportSchema = createInsertSchema(aiSavedReports);

// // AI Analytics Metrics (for tracking AI usage and performance)
// export const aiAnalyticsMetrics = pgTable('ai_analytics_metrics', {
//   id: serial('id').primaryKey(),
//   userId: varchar('user_id', { length: 255 }).notNull(),
//   metricType: varchar('metric_type', { length: 50 }).notNull(), // 'query_count', 'response_time', 'accuracy'
//   metricValue: numeric('metric_value', { precision: 10, scale: 2 }).notNull(),
//   metadata: jsonb('metadata'),
//   recordedAt: timestamp('recorded_at').defaultNow()
// }, (table) => ({
//   userIdIdx: index('idx_ai_metrics_user_id').on(table.userId),
//   typeIdx: index('idx_ai_metrics_type').on(table.metricType),
//   recordedAtIdx: index('idx_ai_metrics_recorded_at').on(table.recordedAt)
// }));

// export const insertAIAnalyticsMetricSchema = createInsertSchema(aiAnalyticsMetrics);

// // Relationships
// export const aiRelations = {
//   conversations: {
//     insights: (conversation: typeof aiConversations) => ({
//       insights: aiInsights,
//       fields: [conversation.id],
//       references: [aiInsights.conversationId]
//     }),
//     feedback: (conversation: typeof aiConversations) => ({
//       feedback: aiFeedback,
//       fields: [conversation.id],
//       references: [aiFeedback.conversationId]
//     })
//   }
// };
...
// export type AIQueryTemplate = typeof aiQueryTemplates.$inferSelect;
// export type NewAIQueryTemplate = typeof aiQueryTemplates.$inferInsert;
...
// export type AISavedReport = typeof aiSavedReports.$inferSelect;
// export type NewAISavedReport = typeof aiSavedReports.$inferInsert;

export const locations = pgTable(
  'locations',
  {
    id: serial('id').primaryKey(), // Make id the primary key
    code: varchar('code', { length: 20 }),
    name: varchar('name', { length: 255 }).notNull().unique(), // Keep name unique but not PK
...
export const customers = pgTable(
  'customers',
  {
    ...
    locationName: varchar('location_name', { length: 255 }).notNull().references(() => locations.name,{ onDelete: 'cascade' }),
    inclusionType: varchar('inclusion_type', { length: 50 }).notNull(),
    additionalParams: integer('additional_params'),
    icon: varchar('icon', { length: 255 }).default('default_icon').references(() => icons.facilityType),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow()
  },
  (table) => {
...
export const icons = pgTable('icons', {
  id: serial('id'),
  facilityType: varchar('facility_type', { length: 50 }).notNull().unique(), // Make unique
  red: integer('red').notNull(),
  green: integer('green').notNull(),
  blue: integer('blue').notNull(),
  createdAt: timestamp('created_at').defaultNow()
});
export const insertIconsSchema = createInsertSchema(icons);
...
export const demandCoverageByDistances = pgTable(
  'demand_coverage_by_distances',
  {
    ...
    siteName: varchar('site_name', { length: 255 }).notNull().unique(), // Keep siteName unique but not PK
    distanceToSiteKm: decimal('distance_to_site_km', {
      precision: 10,
      scale: 2
    }),
    demandPercentage: decimal('demand_percentage', { precision: 8, scale: 2 }),
    demandM3: decimal('demand_m3', { precision: 15, scale: 2 }),
    updatedAt: timestamp('updated_at').defaultNow()
  },
  (table) => ({
...
export const facilities = pgTable(
  'facilities',
  {
    ...
    locationId: integer('location_id')
      .notNull()
      .references(() => locations.id ,{ onDelete: 'cascade' }),
    locationName: varchar('location_name', { length: 255 }).notNull().references(() => locations.name),
...
export const factories = pgTable(
  'factories',
  {
    ...
    // Basic Information
    name: varchar('name', { length: 255 }).notNull().unique(),
    type: varchar('type', { length: 100 }).default('Factory').notNull(),
    ...
    locationName: varchar('location_name', { length: 255 })
      .notNull()
      .references(() => locations.name, { onDelete: 'cascade' }),
    
    // Operational Status
    initiallyOpen: boolean('initially_open').default(true).notNull(),
    inclusionType: varchar('inclusion_type', { length: 50 }).notNull(),
    ...
    // Order Processing
    priority: varchar('priority').default('Equal').notNull(),
    aggregateOrdersByLocation: boolean('aggregate_orders_by_location').default(false).notNull(),
    
    // Additional Parameters for extended objects
    additionalParameters: integer('additional_parameters'),
    
    // Visual Representation
    icon: varchar('icon', { length: 255 })
      .default('FACTORY')
      .references(() => icons.facilityType),
  ...
  (table) => ({
    ...
    nameIdx: index('idx_factory_name').on(table.name),
    locationIdx: index('idx_factory_location').on(table.locationId),
    inclusionTypeIdx: index('idx_factory_inclusion_type').on(table.inclusionType),
    priorityIdx: index('idx_factory_priority').on(table.priority),
    typeIdx: index('idx_factory_type').on(table.type),
    
    // Composite indexes
    nameInclusionIdx: index('idx_factory_name_inclusion').on(table.name, table.inclusionType),
    locationOpenIdx: index('idx_factory_location_open').on(table.locationId, table.initiallyOpen)
  })
);
...
export const demandFulfillment = pgTable(
  'demand_fulfillment',
  {
    ...
    customerName: varchar('customer_name', { length: 255 }).notNull().references(() => customers.name), // Foreign key to Customers table
    productId: integer('product_id').references(() => products.id , { onDelete: 'cascade' }), // Foreign key to Products table
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name), // Measurement unit for the product
...
export const demand = pgTable(
  'demand',
  {
    id: serial('id').primaryKey(), // Add this line
    ...
    timePeriodName: varchar('time_period_name', { length: 255 }).notNull(),
    demandType: varchar('demand_type', { length: 100 }).notNull(),
    parameters: integer('parameters'),
    revenue: decimal('revenue', { precision: 10, scale: 2 }),
    downPenalty: decimal('down_penalty', { precision: 10, scale: 2 }).default('0'),
    ...
    minSplitRatio: decimal('min_split_ratio', { precision: 3, scale: 2 }).default('1.0'),
  ...
  (table) => ({
    uniqueKey: unique().on(table.customerId, table.productId, table.timePeriodId),
    demandTypeCheck: check(
      'demand_type_check',
      sql`${table.demandType} IN ('Periodic demand', 'Periodic demand with first occurrence', 'Historic demand')`
    ),
    minSplitRatioCheck: check(
      'min_split_ratio_check',
      sql`${table.minSplitRatio} > 0 AND ${table.minSplitRatio} < 1`
    ),
    backorderPolicyCheck: check(
      'backorder_policy_check',
      sql`${table.backorderPolicy} IN ('Not Allowed', 'Allowed Total')`
    ),
    inclusionTypeCheck: check(
      'inclusion_type_check',
      sql`${table.inclusionType} IN ('Include', 'Exclude')`
    ),
    customerProductInclusionIdx: index('idx_demand_customer_product_inclusion').on(table.customerId, table.productId, table.inclusionType)
  })
);
...
export const groupCustomers = pgTable(
  'group_customers',
  {
    ...
    groupName: varchar('group_name', { length: 255 }).notNull().references(() => groups.name),
    customerId: integer('customer_id')
      .notNull()
      .references(() => customers.id, { onDelete: 'cascade' }),
    customerName: varchar('customer_name', { length: 255 }).notNull().references(() => customers.name),
  },
  (table) => ({
    pk: primaryKey({ columns: [table.groupId, table.customerId] }),
    indexGroup: index('idx_group_customers_group_id').on(table.groupId)
  })
);
...
export const groupSites = pgTable(
  'group_sites',
  {
    ...
    siteId: integer('site_id')
      .notNull()
      .references(() => facilities.id, { onDelete: 'cascade' }),
    siteName: varchar('site_name', { length: 255 }).notNull().references(() => facilities.name),
  },
  (table) => ({
    pk: primaryKey({ columns: [table.groupId, table.siteId] }),
    indexGroup: index('idx_group_sites_group_id').on(table.groupId)
  })
);
...
export const groupSuppliers = pgTable(
  'group_suppliers',
  {
    ...
    supplierName: varchar('supplier_name', { length: 255 }).notNull().references(() => suppliers.name),
  },
  (table) => ({
...
export const groupGroups = pgTable(
  'group_groups',
  {
    parentGroupId: integer('parent_group_id')
      .notNull()
      .references(() => groups.id, { onDelete: 'cascade' }),
    parentGroupName: varchar('parent_group_name', { length: 255 }).notNull().references(() => groups.name),
    childGroupId: integer('child_group_id')
      .notNull()
      .references(() => groups.id, { onDelete: 'cascade' }),
    childGroupName: varchar('child_group_name', { length: 255 }).notNull().references(() => groups.name),
  },
  (table) => ({
...
export const productGroupProducts = pgTable(
  'product_group_products',
  {
    ...
    productGroupName: varchar('product_group_name', { length: 255 }).notNull(),
    productId: integer('product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }),
    productName: varchar('product_name', { length: 255 }).notNull(),
  ...
  (table) => ({
    ...
    productGroupNameFk: foreignKey({
      columns: [table.productGroupName],
      foreignColumns: [productGroups.name],
      name: 'pgp_group_name_fk'
    }),
    productNameFk: foreignKey({
      columns: [table.productName],
      foreignColumns: [products.name],
      name: 'pgp_product_name_fk'
    })
  })
);
...
export const productFlows = pgTable(
  'product_flows',
  {
    ...
    sourceName: varchar('source_name', { length: 255 }).notNull().references(() => facilities.name), // Foreign key to Facilities table (source)
    ...
    destinationName: varchar('destination_name', { length: 255 }).notNull().references(() => facilities.name), // Foreign key to Facilities table (destination)
    vehicleTypeId: integer('vehicle_type_id').references(() => vehicleTypes.id, { onDelete: 'cascade' }), // Foreign key to Vehicle Types table
    vehicleTypeName: varchar('vehicle_type_name', { length: 255 }).notNull().references(() => vehicleTypes.name), // Foreign key to Vehicle Types table
    productId: integer('product_id').references(() => products.id, { onDelete: 'cascade' }), // Foreign key to Products table
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name), // Foreign key to Products table
    ...
    timePeriodName: varchar('time_period_name', { length: 255 }).notNull().references(() => periods.name), // Foreign key to Periods table
...
export const productStorages = pgTable(
  'product_storages',
  {
    ...
    facilityId: integer('facility_id')
      .notNull()
      .references(() => facilities.id, { onDelete: 'cascade' }),
    facilityName: varchar('facility_name', { length: 255 }).notNull().references(() => facilities.name),
    expandFacilities: boolean('expand_facilities').default(false),
    productId: integer('product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }),
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name),
    ...
    timePeriodName: varchar('time_period_name', { length: 255 }).notNull().references(() => periods.name),
...
export const periodGroupPeriods = pgTable(
  'period_group_periods',
  {
    ...
    periodGroupName: varchar('period_group_name', { length: 255 }).notNull().references(() => periodGroups.name),
    periodId: integer('period_id')
      .notNull()
      .references(() => periods.id, { onDelete: 'cascade' }),
    periodName: varchar('period_name', { length: 255 }).notNull().references(() => periods.name),
  },
  (table) => ({
...
export const suppliers = pgTable(
  'suppliers',
  {
    ...
    additionalParameters: integer('additional_parameters'),
    icon: varchar('icon', { length: 255 }).references(() => icons.facilityType),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow()
  },
  (table) => ({
...
export const sourcing = pgTable(
  'sourcing',
  {
    ...
    deliveryDestination: varchar('delivery_destination', {
      length: 255
    }).notNull().unique(), // Keep deliveryDestination unique but not PK
...
export const indicatorConstraints = pgTable(
  'indicator_constraints',
  {
    ...
    ifConditionName: varchar('if_condition_name', { length: 255 }).notNull().references(() => linearRanges.name),
    thenConditionId: integer('then_condition_id')
      .notNull()
      .references(() => linearRanges.id, { onDelete: 'cascade' }),
    thenConditionName: varchar('then_condition_name', { length: 255 }).notNull().references(() => linearRanges.name),
...
export const paths = pgTable(
  'paths',
  {
    ...
    vehicleTypeName: varchar('vehicle_type_name', { length: 255 }).notNull().references(() => vehicleTypes.name),
...
export const bom = pgTable(
  'bom',
  {
    ...
    endProductId: integer('end_product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }),
    endProductName: varchar('end_product_name', { length: 255 }).notNull().references(() => products.name),
...
export const bomComponents = pgTable(
  'bom_components',
  {
    ...
    bomName: varchar('bom_name', { length: 255 }).notNull().references(() => bom.name),
    productId: integer('product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }),
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name),
...
export const saleBatches = pgTable(
  'sale_batches',
  {
    ...
    sourceId: integer('source_id')
      .notNull()
      .references(() => facilities.id, { onDelete: 'cascade' }),
    sourceName: varchar('source_name', { length: 255 }).notNull().references(() => facilities.name),
    productId: integer('product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }),
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name),
...
export const orderingRules = pgTable(
  'ordering_rules',
  {
    ...
    destinationName: varchar('destination_name', { length: 255 }).notNull().references(() => facilities.name),
    productId: integer('product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }), // Foreign key to Products table
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name),
...
export const paymentTerms = pgTable(
  'payment_terms',
  {
    ...
    sellerName: varchar('seller_name', { length: 255 }).notNull().references(() => facilities.name),
    buyerId: integer('buyer_id')
      .notNull()
      .references(() => facilities.id, { onDelete: 'cascade' }), // Foreign key to Facilities table for the buyer
    buyerName: varchar('buyer_name', { length: 255 }).notNull().references(() => facilities.name),
    productId: integer('product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }), // Foreign key to Products table
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name),
...
export const tariffs = pgTable(
  'tariffs',
  {
    ...
    fromName: varchar('from_name', { length: 255 }).notNull().references(() => facilities.name),
    toId: integer('to_id')
      .notNull()
      .references(() => facilities.id, { onDelete: 'cascade' }), // Foreign key to Facilities table for the destination site
    toName: varchar('to_name', { length: 255 }).notNull().references(() => facilities.name),
    productId: integer('product_id')
      .notNull()
      .references(() => products.id, { onDelete: 'cascade' }), // Foreign key to Products table
    productName: varchar('product_name', { length: 255 }).notNull().references(() => products.name),
...
export const snops = pgTable('snops', {
  id: serial('id').primaryKey(),
  sourceId: integer('source_id').notNull(),
  productId: integer('product_id').notNull(),
  type: varchar('type', { length: 50 }).notNull(),
  batchSize: integer('batch_size').notNull(),
  stepSize: integer('step_size').notNull(),
...
export const insertSnopSchema = createInsertSchema(snops);
export const shippingDestination = pgTable('shipping_destination', {
  id: serial('id').primaryKey(), // Make id the primary key
  shippingId: varchar('shipping_id', { length: 50 }).notNull().unique(), // Keep shipping_id unique but not PK
  contents: varchar('contents', { length: 255 }).notNull(), // Contents field for Ports/DCs/Customers
  createdAt: timestamp('created_at').defaultNow()
  });
  export const insertShippingDestinationSchema = createInsertSchema(shippingDestination);


  export const sourcingSources = pgTable('sourcing_sources', {
    id: serial('id').primaryKey(), // Make id the primary key
    sourcingId: varchar('sourcing_id', { length: 50 }).notNull().unique(), // Keep sourcing_id unique but not PK
    contents: varchar('contents', { length: 255 }).notNull(), // Contents field for DC locations
    createdAt: timestamp('created_at').defaultNow()
    });
    export const insertSourcingSourcesSchema = createInsertSchema(sourcingSources);

    export const netScenario = pgTable(
      'net_scenario',
      {
        id: serial('id').primaryKey(), // Make id the primary key
        netId: varchar('net_id', { length: 50 }).notNull().unique(), // e.g., 'SIM-1', 'SIM-2'
        description: text('description'),
        scenarioType: varchar('scenario_type', { length: 50 }).notNull(),
        created: timestamp('created', { withTimezone: true }).defaultNow().notNull(),
        status: varchar('status', { length: 50 }).default('Not Started').notNull(),
        progress: integer('progress').default(0).notNull(), // 0-100
        
        // Settings columns
        ignoreRoutes: boolean('ignore_routes').default(false).notNull(),
        demandType: varchar('demand_type', { length: 50 }).default('Exact demand').notNull(),
        searchType: varchar('search_type', { length: 50 }).default('Find N best').notNull(),
        bestSolutions: integer('best_solutions').default(1).notNull(),
        timeLimitSec: integer('time_limit_sec').default(600).notNull(),
        mipGap: decimal('mip_gap', { precision: 10, scale: 6 }).default('0.000001').notNull(),
        threads: integer('threads').default(7).notNull(),
        problemType: varchar('problem_type', { length: 50 }).default('Use Big M').notNull(),
        unitType: varchar('unit_type', { length: 50 }).default('m³').notNull(),
        distanceType: varchar('distance_type', { length: 50 }).default('km').notNull(),
        currency: varchar('currency', { length: 50 }).default('USD').notNull(),
        
        // Solution results (populated after solving)
        solutionPool: text('solution_pool'), // e.g., "Solution pool", "95% - 100%"
        objectiveValue: decimal('objective_value', { precision: 20, scale: 6 }),
        solveTimeSec: decimal('solve_time_sec', { precision: 10, scale: 3 }),
        iterations: integer('iterations'),
        gap: decimal('gap', { precision: 10, scale: 6 }),
        
        // Metadata
        updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
        metadata: text('metadata').$type<Record<string, any>>() // Additional data as JSONB
      },
      (table) => ({
        // Constraints
        progressCheck: sql`CHECK (${table.progress} >= 0 AND ${table.progress} <= 100)`,
        bestSolutionsCheck: sql`CHECK (${table.bestSolutions} > 0)`,
        timeLimitCheck: sql`CHECK (${table.timeLimitSec} > 0)`,
        threadsCheck: sql`CHECK (${table.threads} > 0)`,
        
        // Indexes
        netIdIdx: index('idx_net_scenario_net_id').on(table.netId),
        statusIdx: index('idx_net_scenario_status').on(table.status),
        createdIdx: index('idx_net_scenario_created').on(table.created),
        scenarioTypeIdx: index('idx_net_scenario_type').on(table.scenarioType),
        statusProgressIdx: index('idx_net_scenario_status_progress').on(table.status, table.progress)
      })
    );
export const insertNetScenarioSchema = createInsertSchema(netScenario);